...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (sessionId) {
      await trackEndpointUsage(sessionId, 'agent/general')
    }
    ...
    const customStream = createAgentStreamHandler(agentStream, {
      ...
      onFinish: async () => {
        ...
        updateTodayStats(user.id).catch((err) => {
          devError('[General Agent] Failed to update daily stats:', err)
        })
      },
      onError: (error) => {
    ...
    devError('❌ [GENERAL AGENT] Error nach', requestTime + 'ms:', error)
    ...
    try {
      ...
      devError('[GENERAL AGENT] Failed to log error:', logError)
    }
